...
public class UserService {
    ...
    public UserDetail findByUserId(String id) {
        try {
            GetResponse<UserEntity> response = client.get(requestGet -> requestGet
                            .index(indexName)
                            .id(id),
                    UserEntity.class);
...
// todo: stream and paralelstream difference
            ...
            long count = response.items()
                    .parallelStream().filter(item -> item.error() == null).count();
            if (response.errors()) {
                CompletableFuture.runAsync(() -> {
                    response.items().forEach(item -> {
                        if (item.error() != null) {
                            log.warn("Error indexing user ID {}: {}", item.id(), item.error().reason());
                        }
                    });
                });
            }
        ...
        try {
            SearchResponse<Void> response = client.search(builder -> builder
                    ...
                    .query(queryBuilder -> queryBuilder
                            .matchAll(matchAllBuilder -> matchAllBuilder)), Void.class);
