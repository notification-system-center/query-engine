...
class UserServiceTest {
  ...
  void shouldReturnUserDetail_whenUserExists() throws IOException {
    ...
    GetResponse<UserEntity> response =
        GetResponse.of(
            builder -> builder.id(ID.toString()).index(INDEX_NAME).found(true).source(userEntity));
  ...
  void shouldReturnNotFoundException_whenUserDoesNotExist() throws IOException {
    // GIVEN
    GetResponse<UserEntity> response =
        GetResponse.of(builder -> builder.id(ID.toString()).index(INDEX_NAME).found(false));

    String expectedExceptionMessage =
        MessageFormat.format(ExceptionMessageEnum.NOT_FOUND_EXCEPTION.getMessage(), ID);
    // WHEN
    when(client.get(any(Function.class), eq(UserEntity.class))).thenReturn(response);

    // THEN
    Exception ex =
        assertThrows(NotFoundException.class, () -> userService.findByUserId(ID.toString()));
  ...
  void saveAll_whenFileIsFull_and_NoErrors() throws IOException {
    ...
    BulkResponse bulkResponse =
        new BulkResponse.Builder().items(List.of(item)).errors(false).took(10L).build();
  ...
  void saveAll_whenFileIsFull_and_hasSomeErrors() throws IOException {
    ...
    BulkResponseItem itemWithError =
        new BulkResponseItem.Builder()
            .index(INDEX_NAME)
            .id(ID.toString())
            .operationType(OperationType.Index)
            .status(400)
            .result("error")
            .error(error -> error.type("index failed").reason("Failed to index document"))
            .build();

    BulkResponseItem itemWithSuccess =
        new BulkResponseItem.Builder()
            .index(INDEX_NAME)
            .id(ID.toString())
            .operationType(OperationType.Index)
  ...
  void isIndexEmpty_ShouldReturnTrue_WhenIndexHasNoDocuments() throws IOException {
    // Given
    SearchResponse<Void> emptyResponse =
        SearchResponse.of(
            voidBuilder ->
                voidBuilder
                    .took(1)
                    .timedOut(false)
                    .shards(builder -> builder.total(1).successful(1).failed(0))
                        ...
                        builder ->
                            builder.hits(
                                Collections.singletonList(
                                    Hit.of(
                                        hitBuilder ->
                                            hitBuilder.id(ID.toString()).index(INDEX_NAME))))));
    // when
